...
def model_eval_multitask(sentiment_dataloader,
    ...
    with torch.no_grad():
        ...
        if enable_sts:
            ...
            sts_accuracy = pearson_mat[1][0]


        return (sentiment_accuracy,sst_y_pred, sst_sent_ids,
                paraphrase_accuracy, para_y_pred, para_sent_ids,
                sts_accuracy, sts_y_pred, sts_sent_ids)


def models_eval_multitask(sentiment_dataloader,
                         paraphrase_dataloader,
                         sts_dataloader,
                         models,
                         device,
    ...
    with torch.no_grad():
        ...
        if enable_sst:
            for step, batch in enumerate(tqdm(sentiment_dataloader, desc=f'eval', disable=TQDM_DISABLE)):
                ...
                y_hat_final = np.apply_along_axis(lambda x: np.bincount(x.astype(int)).argmax(), axis=0, arr=predictions)

                print(y_hat_final, '==== y_hat_final =====')
                
                
                b_labels = b_labels.flatten().cpu().numpy()

                sst_y_pred.extend(y_hat_final)
                sst_y_true.extend(b_labels)
                sst_sent_ids.extend(b_sent_ids)

            sentiment_accuracy = np.mean(np.array(sst_y_pred) == np.array(sst_y_true))

            writeLogIntoFileAndPrint(f'{sentiment_accuracy:.2f} ======= sentiment_accuracy ======')
        ...
        if enable_para:
            ...
            for step, batch in enumerate(tqdm(paraphrase_dataloader, desc=f'eval', disable=TQDM_DISABLE)):
                ...
                para_y_pred.extend(y_hat_final)
                para_y_true.extend(b_labels)
                para_sent_ids.extend(b_sent_ids)

            paraphrase_accuracy = np.mean(np.array(para_y_pred) == np.array(para_y_true))

            writeLogIntoFileAndPrint(f'{paraphrase_accuracy:.2f} ======= paraphrase_accuracy ======')
        ...
        if enable_sts:
            ...
            for step, batch in enumerate(tqdm(sts_dataloader, desc=f'eval', disable=TQDM_DISABLE)):
                ...
                y_hat_final = np.mean(predictions, axis=0)

                b_labels = b_labels.flatten().cpu().numpy()
                
                sts_y_pred.extend(y_hat_final)
                sts_y_true.extend(b_labels)
                sts_sent_ids.extend(b_sent_ids)
            pearson_mat = np.corrcoef(sts_y_pred,sts_y_true)
            sts_accuracy = pearson_mat[1][0]

            writeLogIntoFileAndPrint(f'{sts_accuracy:.2f} ======= sts_accuracy ======')
